...
public class IntSuchbaum {
    ...
    public Folge<Integer> breitensuche() {
        ...
        Folge<Integer> result = new FolgeMitRing<>(this.size());
        Schlange<TreeElement> schlange = new SchlangeMitEVL<>();

        if (root != null) {
            schlange.insert(root);
        }

        while (!schlange.isEmpty()) {
            TreeElement current = schlange.remove();
            result.insert(current.data);

            if (current.left != null) {
                schlange.insert(current.left);
            }
            if (current.right != null) {
                schlange.insert(current.right);
            }
        }
        return result;
    }
